...
```shell
# clone or download project archive from repository
...
- Для того, чтобы ускорить процесс разработки использовал `Tailwind` & `shadcn/ui` в качестве UI инструментов. Сделал шаблон страницы в Figma (скриншот ниже)
...
- Подумал, как можно было бы частично избавиться от стейта и оставить input в состоянии *uncontrolled* + отдавать актуальное значение в `server action` через атрибут `name`. Это не так просто, так как почти на каждом input своя валидация и форматирование для пользователя
...
- Добавил прелоад для изображений, чтобы отображались моментально
