...
- Подумал, как можно было бы частично избавиться от стейта и оставить input в состоянии _uncontrolled_ + отдавать актуальное значение в `server action` через атрибут `name`. Это не так просто, так как почти на каждом input своя валидация и форматирование для пользователя
